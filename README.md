...
Flowbite Blog
Provided by GitHub Copilot Chat Assistant
...
Quickstart — Development
Clone repository git clone https://github.com/arshprem01/flowbite-blog.git cd flowbite-blog

Install Ruby dependencies gem install bundler bundle install

Install Node dependencies (for PostCSS/Tailwind) npm install

Setup database (development) bin/rails db:create db:migrate db:seed

Run development processes (Tailwind watcher + Rails) ./bin/dev
...
If you prefer to run Rails server directly: bin/rails server -b 0.0.0.0
...
Run Bundler Audit: bundle-audit check --update

Docker (production build)
Build image: docker build -t flowbite-blog:latest .

Run container (example): docker run --rm -e RAILS_ENV=production -e SECRET_KEY_BASE=your_secret -p 3000:3000 flowbite-blog:latest
...
Example multi-step for production container: docker build -t flowbite-blog:latest . docker run --rm -e RAILS_ENV=production -e DATABASE_URL=postgres://user:pass@db:5432/mydb -e SECRET_KEY_BASE=... -p 3000:3000 flowbite-blog:latest

Deployment
Kamal (included as a gem) — recommended for containerized deployment and edge cases:

See kamal-deploy.org for setup.
